execute as @p at @s unless entity @s[tag=menu] if entity @s[nbt={SelectedItem:{tag:{menu:1b}}}] run summon chest_minecart ~ ~1 ~ {Invulnerable:1b,NoGravity:1b,CustomName:'{"text":"Menu"}',CustomNameVisible:false,CustomDisplayTile:1b}
